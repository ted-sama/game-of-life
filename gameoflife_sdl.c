...
SDLContext* init_sdl(int rows, int cols, int speed) {
    ...
    if (context->cell_size > MAX_CELL_SIZE) context->cell_size = MAX_CELL_SIZE;
...
void render_board(SDLContext *context, Board *board) {
    if (!context || !board || !context->renderer) return;
    ...
    for (int i = 0; i < board->rows; i++) {
        for (int j = 0; j < board->cols; j++) {
            SDL_Rect cell = {
                base_x + (j * effective_cell_size),
                base_y + (i * effective_cell_size),
                effective_cell_size - 1,
                effective_cell_size - 1
            };
...
void handle_events(SDLContext *context, Board *board) {
    ...
    if (keyboard[SDL_SCANCODE_UP]) context->offset_y += PAN_SPEED;
    if (keyboard[SDL_SCANCODE_DOWN]) context->offset_y -= PAN_SPEED;
    if (keyboard[SDL_SCANCODE_LEFT]) context->offset_x += PAN_SPEED;
    if (keyboard[SDL_SCANCODE_RIGHT]) context->offset_x -= PAN_SPEED;
        ...
        switch (event.type) {
            ...
            case SDL_MOUSEWHEEL: // Zoom
                if (event.wheel.y > 0) {
                    ...
                    context->zoom *= 0.9f;
                    if (context->zoom < 0.2f) context->zoom = 0.2f;
                }
                break;
        }
    }
